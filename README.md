...
- Git commit：`ca473c0`（`main`）
- 应用版本：`v20260824.200106`
- Service Worker 缓存：`rabbittodo-v115`
...
- 底部采用 56px 高、28px 圆角、最大宽度 280px 的紧凑悬浮毛玻璃胶囊，底部位于 Home Indicator 安全区上方 6px。玻璃使用 24% 白色底、20px 模糊、150% 饱和度、30% 白色边框与极轻阴影；任务内容可滚动到胶囊背后并被实时模糊，列表末尾保留胶囊高度加 18px 的补偿空间。待办、已办使用无描边的 72 × 44px 淡灰色内胶囊表示选中，图标与文字仍使用主题色；44px 主题色圆形“新增”按钮位于 88px 宽的中央栏，我的页不显示新增按钮。横屏任务页的胶囊以右侧任务列表居中，横屏切回竖屏时自动收起筛选区。iOS 主屏 PWA 使用包含安全区的 `100vh` 填满物理视口；iOS 浏览器保留动态高度，但把胶囊固定到可视视口底部。iOS 判断同时校验真实设备，避免 Mac PWA 被误套用移动端修复。真实手机与 iPad 页面不模拟设备四角圆角，由系统视口负责屏幕裁切；34px 外框圆角只用于桌面机框预览。
