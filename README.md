...
- Git commit：`4cb57fd`（`main`）
- 应用版本：`v20260824.220156`
- Service Worker 缓存：`rabbittodo-v116`
...
- 底部采用 56px 高、28px 圆角、最大宽度 280px 的紧凑悬浮毛玻璃胶囊。浏览器动态视口底部留 6px，standalone 模式额外叠加 Home Indicator 安全区。玻璃使用 24% 白色底、20px 模糊、150% 饱和度、30% 白色边框与极轻阴影；任务内容可滚动到胶囊背后并被实时模糊，列表末尾保留胶囊高度加 18px 的补偿空间。待办、已办使用无描边的 72 × 44px 淡灰色内胶囊表示选中，图标与文字仍使用主题色；44px 主题色圆形“新增”按钮位于 88px 宽的中央栏，我的页不显示新增按钮。内容和 dock 覆盖层共享同一个 CSS Grid 单元，不使用绝对或固定定位；横屏 dock 与工作区复用相同网格列，胶囊自然位于右侧任务列表中央。横屏切回竖屏时自动收起筛选区。WebKit 触屏主屏应用仅通过 CSS 能力和运行模式查询切换到包含安全区的 `100vh`，不在 JavaScript 中枚举 iOS、Android、Mac 或浏览器。真实手机与 iPad 页面不模拟设备四角圆角，由系统视口负责屏幕裁切；34px 外框圆角只用于桌面机框预览。
