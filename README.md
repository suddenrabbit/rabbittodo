...
前端会在回到前台时同步，并在可见状态下每 30 秒拉取一次任务。新增、编辑、完成、删除与排序采用乐观更新：界面立即响应，数据库写入在后台按操作顺序串行完成；若写入失败，应用会提示并恢复服务器中的最新数据。对于个人或少量设备使用，这只是很小的 D1 读取负载；Worker 端保持了 `Cache-Control: no-store`，避免任务数据被缓存为旧内容。
