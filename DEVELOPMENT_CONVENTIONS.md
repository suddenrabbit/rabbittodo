...
- WebKit 触屏主屏应用通过现有 CSS 能力、standalone 和粗指针查询，将任务/我的页顶部设为安全区加 20px（保留 iPad 原有最小间距），避让系统渐变尾部。保留 iOS 26 所需的 `100vh` 修复，不按系统版本判断；旧版 iOS 主屏模式也会命中，实际间距及 iOS 27 渐变仍需真机验证。
