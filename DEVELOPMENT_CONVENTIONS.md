...
- 界面通过 `prefers-color-scheme` 自动跟随系统浅色/深色外观，不增加独立模式开关。深色模式采用深灰分层而非纯黑反转：页面、卡片、控件、弹窗和毛玻璃分别使用语义表面色；普通文字、次级文字、边框、阴影及错误/逾期/提醒等状态色均须单独校正。五种账号主题值保持不变，但在深色环境使用更明亮的强调文字和较深的操作填充色；任务自身颜色继续独立。系统切换时应即时响应，HTML 初始背景及浅/深 `theme-color` 必须同步，避免启动白闪。不涉及数据库字段或账号偏好迁移。
