...
- 当前 2.6 本地开发基线：commit `f12c32f`，应用版本 `v20260822.091737`，Service Worker `rabbittodo-v108`。`migrations/0010_account_task_sort_mode.sql` 已在本地与生产 D1 应用。
...
- 底栏为 56px（另加安全区）的全宽毛玻璃导航；安全区外上、下留白分别为 4px、2px，待办与已办按钮使用 56 × 44px 触控区。底栏悬浮覆盖在滚动内容之上，任务内容须延伸到底栏背后以形成真实背景模糊，列表末尾必须保留底栏高度加 12px 的滚动补偿，确保最后一项可完整避让。待办、已办页将 44px 主题色圆形新增按钮收入中间栏位，我的页不显示新增按钮。真实手机与 iPad 的应用容器不得硬编码屏幕四角圆角，保持矩形视口并由设备系统裁切；布局使用 `safe-area-inset-*` 为物理圆角、刘海与 Home Indicator 预留空间，桌面机框预览可保留模拟圆角。已办页左上角小品牌显示 `RabbitDone`，待办与我的页保持 `RabbitToDo`。
