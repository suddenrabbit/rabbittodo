...
- 当前 2.0 基线：commit `e2498f7`，应用版本 `v20260801.110605`，Service Worker `rabbittodo-v43`
...
- 没有 schema 变化时不要执行无关 SQL；任务内容密文本身继续复用现有 TEXT 字段。账户体系 migration `0006_user_accounts.sql` 已进入 2.0 生产基线；2.1 的 `0007_offline_task_mutations.sql` 仅向任务表增加可空的离线创建幂等编号与唯一索引，保留全部既有任务；后续结构变化从新的 migration 编号继续。
...
- 会话 Cookie 为 HttpOnly、SameSite=Strict，闲置 180 天过期；每次前台会话恢复自动续期，多个设备可同时登录。改密码、管理员重置和禁用账号仍撤销会话。
- 启动必须先恢复本地任务快照，再拉取远端权威数据；待确认的 Service Worker 更新不能阻塞远端同步。新增和编辑每次都直接发送远端，不得用 `navigator.onLine` 或其他浏览器网络状态字段跳过请求；只有超时或网络层失败才进入本地 outbox 且显示“未同步”。遗留 outbox 不得阻塞远端读取，读取时需合并保留本机未同步项；前台恢复、网络状态变化和定时检查持续重试，成功后清除标记。创建操作使用持久客户端编号防止重试重复创建，跨设备同一任务冲突按最后成功同步的写入覆盖。
- 竖屏任务页固定标题与筛选区，只允许 `.workspace-tasks` 独立滚动；横屏继续由 `.content-scroll` 承担双栏整体滚动。重新渲染同一视图/筛选结果时必须恢复任务滚动锚点，切换视图或筛选条件时才从新列表顶部开始。
