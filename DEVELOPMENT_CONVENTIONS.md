...
- 用户名只用于登录和展示；既有用户的内部身份与固定加密种子仍为原 6 位身份码，新用户使用随机内部身份与独立的 256 位随机加密种子。
- 密码用于认证并在浏览器中解锁固定加密种子。修改密码和管理员一次性重置都只重新包裹同一个种子，不能改写任务密文。
- 为支持管理员重置，本地和生产都必须显式配置至少 32 位的 `SERVER_MASTER_KEY`；生产值必须稳定、高熵并妥善备份，服务端不得保留固定主密钥回退。
