...
- 当前 2.6 本地开发基线：commit `4cb57fd`，应用版本 `v20260824.220156`，Service Worker `rabbittodo-v116`。`migrations/0010_account_task_sort_mode.sql` 已在本地与生产 D1 应用。
...
- WebKit 触屏主屏应用通过现有 CSS 能力、standalone 和粗指针查询，将统一顶部内容间距设为安全区加 20px，浏览器使用安全区加 14px，iPad 保留 42px 最小间距；横屏由共同内容容器统一避让，避免左右列重复计算。视口高度统一使用 `100vh` 回退、支持时切换 `100svh`/`100dvh`，WebKit standalone 主屏应用再覆盖为 `100vh` 以保留 iOS 26 兼容。系统渐变仍由系统绘制，实际 iOS 26 及更低、iOS 27、旧图标与新安装需真机验证。
...
- 底栏采用分离式悬浮 dock：左侧双 tab Liquid Glass 胶囊自适应占视口约 60%–70%（常规手机约 220–262px），右侧是独立的 56px 主题色玻璃透镜新增按钮，两者间距 12px；浏览器动态视口底部留 6px，standalone 模式通过统一变量额外叠加 `safe-area-inset-bottom`。左侧表面必须由透明底层、非对称折射高光、渐变边缘、内外阴影和 `backdrop-filter` 共同形成，不得退化为单层白色透明底；待办与已办各占胶囊一半，图标和文字横向排列，选中项使用单一中性透明内镜片，切换时通过 FLIP 位移动画横向滑动并带轻微回弹，主题色只作用于图标、文字与极弱辉光。新增按钮的外层保持背景可透，内部以径向高光、主题色色散及焦散光晕建立主操作层级，加号固定为白色且不得出现完整实色描边；按压时高光和焦散轻微位移，所有动效均须遵循 `prefers-reduced-motion`。我的页不显示新增按钮。内容区与 dock 覆盖层必须共享 `.phone` 的同一个 CSS Grid 单元，不得用绝对或固定定位实现悬浮；横屏 dock 与工作区复用相同网格轨道变量，使整组 dock 自然位于右侧任务列表中央，不使用 JavaScript 测量位置。横屏切回竖屏时自动收起筛选区。任务内容须延伸到 dock 及底部安全区背后，列表末尾保留 `74px + safe-area-inset-bottom` 的滚动补偿。WebKit 触屏主屏应用通过 `@supports`、`display-mode: standalone` 和粗指针能力查询切换到包含安全区的 `100vh`；不得在 JavaScript 中按 iPhone、iPad、Android、Mac 或具体浏览器枚举底栏布局。真实手机与 iPad 的应用容器不得硬编码屏幕四角圆角，保持矩形视口并由设备系统裁切，桌面机框预览可保留模拟圆角。不实现随滚动自动收缩。已办页左上角小品牌显示 `RabbitDone`，待办与我的页保持 `RabbitToDo`。
- 外观模式提供“自动、浅色、深色”三态控件并放在“我的”页；偏好仅写入当前设备的 `localStorage` 键 `rabbittodo-appearance-mode`，有效值为 `system | light | dark`，非法值回退 `system`，退出登录不得清除。启动脚本必须在样式加载前解析实际外观并设置根节点 `data-appearance-mode` 与 `data-color-scheme`，同步 HTML 初始背景及 `theme-color`，避免白闪；系统变化仅在自动模式下即时生效，管理后台沿用同一设备偏好。深色模式采用深灰语义层级而非纯黑反转，覆盖页面、卡片、控件、弹窗和玻璃层；五种账号主题及任务颜色保持独立。不新增数据库字段、账号偏好接口或迁移。
