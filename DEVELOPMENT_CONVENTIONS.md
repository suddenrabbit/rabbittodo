...
- 当前 2.6 本地开发基线：commit `76b6e71`，应用版本 `v20260822.084017`，Service Worker `rabbittodo-v104`。`migrations/0010_account_task_sort_mode.sql` 已在本地与生产 D1 应用。
...
- 底栏为 64px（另加安全区）的全宽普通导航；待办、已办页将 44px 主题色圆形新增按钮收入中间栏位，我的页不显示新增按钮。已办页左上角小品牌显示 `RabbitDone`，待办与我的页保持 `RabbitToDo`。
