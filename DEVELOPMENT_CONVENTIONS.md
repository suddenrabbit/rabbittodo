...
- 当前 2.6 本地开发基线：commit `ca473c0`，应用版本 `v20260824.200106`，Service Worker `rabbittodo-v115`。`migrations/0010_account_task_sort_mode.sql` 已在本地与生产 D1 应用。
...
- 底栏为 56px 高、28px 圆角、最大宽度 280px 的紧凑悬浮毛玻璃胶囊，玻璃使用 24% 白色底、20px 模糊、150% 饱和度、30% 白色 1px 边框与极轻阴影，胶囊底部位于 `safe-area-inset-bottom + 6px`。待办与已办按钮基础触控区为 56 × 44px，选中项扩展为无描边的 72 × 44px 中性淡灰色内胶囊，图标与文字仍使用主题色；44px 主题色圆形新增按钮位于 88px 宽的中央栏，我的页不显示新增按钮。横屏任务页的胶囊须按右侧任务列表区域居中，横屏切回竖屏时自动收起筛选区。任务内容须延伸到胶囊及底部安全区背后，列表末尾保留 `74px + safe-area-inset-bottom` 的滚动补偿。iOS 主屏 standalone 模式使用包含安全区的 `100vh` 作为根容器高度；iOS 浏览器保留 `dvh` 并将胶囊固定到可视视口底部。iOS 专用分支必须同时校验 iPhone/iPad 设备与运行模式，不能仅凭 `navigator.standalone` 判断，以免误伤 Mac PWA；Mac Safari 和 Mac PWA维持桌面视口逻辑。真实手机与 iPad 的应用容器不得硬编码屏幕四角圆角，保持矩形视口并由设备系统裁切，桌面机框预览可保留模拟圆角。不实现随滚动自动收缩。已办页左上角小品牌显示 `RabbitDone`，待办与我的页保持 `RabbitToDo`。
