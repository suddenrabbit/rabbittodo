...
- 底栏是 56px（另加安全区）的全宽毛玻璃导航；安全区外上、下留白分别为 4px、2px，任务内容可滚动到其背后形成真实模糊，列表末尾保留底栏高度加 12px 的避让空间。待办、已办按钮触控区为 56 × 44px，中间有 44px 主题色圆形新增按钮，我的页不显示新增按钮。真实手机与 iPad 不硬编码屏幕四角圆角，由系统视口裁切并通过四向安全区预留物理边缘空间；桌面机框预览保留模拟圆角。
