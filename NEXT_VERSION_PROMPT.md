...
- 底栏采用分离式悬浮 dock：左侧双 tab Liquid Glass 胶囊自适应占视口约 60%–70%（常规手机约 220–262px），右侧是独立的 56px 主题色玻璃透镜新增按钮，两者间距 12px；浏览器动态视口底部留 6px，standalone 模式额外叠加底部安全区。左侧材质由透明底层、非对称折射高光、渐变边缘、内外阴影与背景模糊共同形成，选中项是单一中性透明内镜片，切换时通过 FLIP 位移动画横向滑动并轻微回弹，主题色只用于图标、文字和极弱辉光。新增按钮保持背景可透，以径向反光、主题色色散和焦散光晕强调主操作，加号固定为白色且没有完整实色描边；按压时高光与焦散轻微位移，所有动效均遵循减弱动态设置。我的页不显示新增按钮。任务内容可滚动到 dock 及安全区背后，列表末尾保留 `74px + safe-area-inset-bottom` 的避让空间。内容与 dock 共享 `.phone` 的同一 CSS Grid 单元，横屏 dock 与工作区复用网格轨道并使整组 dock 自然在右侧任务列表居中，不使用绝对、固定定位或 JavaScript 测量。横屏切回竖屏时自动收起筛选区；WebKit 触屏主屏应用通过 CSS 能力和运行模式查询使用包含安全区的 `100vh`，不按设备或浏览器枚举。不实现滚动自动收缩。真实设备不硬编码屏幕四角圆角，桌面机框预览保留模拟圆角。
- 明暗外观在“我的”页提供“自动、浅色、深色”三态控件，仅以 `rabbittodo-appearance-mode` 保存在当前设备；启动时在样式加载前解析为根节点 `data-color-scheme`，同步页面初始背景和 `theme-color`。自动模式跟随系统即时切换，手动模式不受系统变化影响，管理后台沿用同一设备选择；不增加数据库字段或账号接口。深色模式使用深灰语义层级并覆盖全部主要表面，五种账号主题、任务颜色和语义状态色保持独立。
